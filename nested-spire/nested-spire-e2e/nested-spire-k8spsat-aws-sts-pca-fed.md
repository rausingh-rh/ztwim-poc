...
> **Note:** Cluster hostnames below are from ephemeral test clusters used during this specific verification run. They will not exist after teardown. Replace with your own cluster domains when reproducing.
...
└────────────────────────────────────────────────────────────────────┘
      ...
      │                              ↕
┌─── Spoke Cluster (GCP) ─────┐  ┌─── Fed Cluster (GCP) ─────────────┐
...
```bash
export HUB_KUBECONFIG=<path-to-hub-cluster-kubeconfig>
export SPOKE_KUBECONFIG=<path-to-spoke-cluster-kubeconfig>
export FED_KUBECONFIG=<path-to-fed-cluster-kubeconfig>
export AWS_ACCOUNT_ID=<your-aws-account-id>
```

| Role  | Kubeconfig         | Platform                          | Notes                                                             |
| ----- | ------------------ | --------------------------------- | ----------------------------------------------------------------- |
| Hub   | `$HUB_KUBECONFIG`  | AWS (STS-enabled, IPI, us-east-1) | OCP 4.20+, uses AWS PCA via cert-manager + IRSA (no static creds) |
| Spoke | `$SPOKE_KUBECONFIG`| GCP                               | Nested SPIRE downstream of Hub                                    |
| Fed   | `$FED_KUBECONFIG`  | GCP                               | Independent, federation with Hub                                  |

### Discover Cluster Domains

Run this once to confirm your cluster domains are resolvable:

```bash
echo "=== Hub (AWS STS) ==="
KUBECONFIG=$HUB_KUBECONFIG oc get dns cluster -o jsonpath='{.spec.baseDomain}'
echo ""

echo "=== Spoke (GCP) ==="
KUBECONFIG=$SPOKE_KUBECONFIG oc get dns cluster -o jsonpath='{.spec.baseDomain}'
echo ""

echo "=== Fed (GCP) ==="
KUBECONFIG=$FED_KUBECONFIG oc get dns cluster -o jsonpath='{.spec.baseDomain}'
echo ""
```

Note down the output — the trust domains will be `apps.<baseDomain>` for each cluster.


## Prerequisites

- Three OpenShift clusters with `oc` CLI access (Hub on AWS STS, Spoke + Fed on GCP)
...
- IAM permissions: `iam:CreatePolicy`, `iam:CreateRole`, `iam:AttachRolePolicy` (one-time setup by admin)
- **No certificates needed for nested SPIRE** (k8s_psat is entirely token-based; https_spiffe uses SPIRE's own SVIDs)
- **IRSA-compatible cluster** — The Hub must be STS-enabled (`credentialsMode: Manual`) with an OIDC provider registered in AWS IAM

> **STS Cluster + IRSA:** The Hub cluster is STS-enabled with an OIDC provider registered in AWS IAM. We leverage this to give the aws-privateca-issuer pod **short-lived STS tokens** via IRSA (IAM Roles for Service Accounts). No static credentials are stored in the cluster — the pod assumes an IAM role using its projected ServiceAccount token. This is the AWS best practice for pod-level AWS access.
...
> **Skip this phase** if you already have an active AWS Private CA from a previous run. Just set `PCA_ARN` and `PCA_FINGERPRINT` to the existing values.
...
#### 1d. Create IAM Role with IRSA for PCA Access (STS Best Practice)

Since the Hub is STS-enabled (with OIDC provider), we use **IRSA (IAM Roles for Service Accounts)** — the pod gets short-lived STS tokens automatically. No static credentials, no Secrets, no rotation.

**How it works:** The cluster's OIDC provider is registered with AWS IAM. When a pod with the right ServiceAccount annotation runs, the AWS SDK in the pod:
1. Reads a projected service account token from the filesystem
2. Calls `sts:AssumeRoleWithWebIdentity` with that token
3. Gets back temporary AWS credentials (auto-refreshed)
...
# Expected: <your-cluster-infra-name>-oidc.s3.<region>.amazonaws.com

# Step 1: Create the IAM policy scoped to the specific PCA
...
# Step 2: Create IAM role with trust policy for the issuer pod's ServiceAccount
cat > /tmp/pca-trust-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "${OIDC_PROVIDER}:sub": "system:serviceaccount:aws-privateca-issuer:aws-pca-issuer-aws-privateca-issuer"
        }
      }
    }
  ]
...
ROLE_ARN=$(aws iam create-role \
  --role-name spire-pca-issuer-role \
  --assume-role-policy-document file:///tmp/pca-trust-policy.json \
  --query 'Role.Arn' --output text)

echo "ROLE_ARN: $ROLE_ARN"

# Step 3: Attach the PCA policy to the role
aws iam attach-role-policy \
  --role-name spire-pca-issuer-role \
  --policy-arn "$POLICY_ARN"

echo "IAM role created with IRSA trust. No static credentials needed."
```

> **What the trust policy says:** "Only a pod running as ServiceAccount `aws-pca-issuer-aws-privateca-issuer` in namespace `aws-privateca-issuer` on THIS specific cluster can assume this role." This is the tightest possible scoping — no other pod, namespace, or cluster can use these PCA permissions.
...
Verify operator is running on each cluster:

```bash
for kc in $HUB_KUBECONFIG $SPOKE_KUBECONFIG $FED_KUBECONFIG; do
  echo "=== Cluster: $kc ==="
  KUBECONFIG=$kc oc get csv -n zero-trust-workload-identity-manager | grep zero-trust
  echo ""
done
```
...
Authentication uses **IRSA** (IAM Roles for Service Accounts) — the pod receives short-lived STS tokens via a projected ServiceAccount token. No static credentials are stored in the cluster.
...
# 4b. Install via Helm with IRSA ServiceAccount annotation
...
# 4c. Set the AWS region and configure STS token projection
oc set env deployment/aws-pca-issuer-aws-privateca-issuer \
  -n aws-privateca-issuer \
  AWS_REGION=${AWS_REGION}

# 4d. Mount projected ServiceAccount token for STS authentication
# The AWS SDK reads from AWS_WEB_IDENTITY_TOKEN_FILE to call sts:AssumeRoleWithWebIdentity
# NOTE: Use strategic merge (not JSON patch) because volumes/volumeMounts may not exist yet
oc patch deployment aws-pca-issuer-aws-privateca-issuer \
  -n aws-privateca-issuer --type=strategic -p '{
  "spec": {
    "template": {
      "spec": {
        "volumes": [{
        ...
        "containers": [{
          ...
          "env": [
            {"name": "AWS_WEB_IDENTITY_TOKEN_FILE", "value": "/var/run/secrets/sts.amazonaws.com/serviceaccount/token"},
...
# 4e. Grant SCC permissions (OpenShift-specific)
...
# 4f. Disable approval check (required for cert-manager v1.3+)
oc patch deployment aws-pca-issuer-aws-privateca-issuer \
  -n aws-privateca-issuer --type=json -p='[
  {"op":"add","path":"/spec/template/spec/containers/0/args","value":["--disable-approved-check"]}
]'

# 4g. Verify the plugin is running and using STS
oc get pods -n aws-privateca-issuer
# Expected: 1/1 Running

# Verify IRSA is configured correctly
oc get deployment aws-pca-issuer-aws-privateca-issuer -n aws-privateca-issuer \
  -o jsonpath='{.spec.template.spec.containers[0].env}' | python3 -m json.tool | grep -A1 "AWS_ROLE_ARN\|AWS_WEB_IDENTITY"
# Expected: AWS_ROLE_ARN and AWS_WEB_IDENTITY_TOKEN_FILE are set

echo "aws-privateca-issuer is ready (using STS/IRSA — no static credentials)."
```
...
for kc in $HUB_KUBECONFIG $SPOKE_KUBECONFIG $FED_KUBECONFIG; do
  echo "=== Cluster: $kc ==="
  KUBECONFIG=$kc oc exec spire-server-0 -c spire-server -n zero-trust-workload-identity-manager -- /spire-server healthcheck
  echo ""
done
# Expected: "Server is healthy." for each
...
echo "Waiting for Hub operator restart..."
KUBECONFIG=$HUB_KUBECONFIG oc rollout status deployment -n zero-trust-workload-identity-manager -l app.kubernetes.io/name=zero-trust-workload-identity-manager --timeout=60s 2>/dev/null || true
sleep 10

echo "Waiting for Spoke operator restart..."
KUBECONFIG=$SPOKE_KUBECONFIG oc rollout status deployment -n zero-trust-workload-identity-manager -l app.kubernetes.io/name=zero-trust-workload-identity-manager --timeout=60s 2>/dev/null || true
sleep 10
```
...
oc rollout status statefulset spire-server -n zero-trust-workload-identity-manager --timeout=120s
...
```bash
# Wait for upstream agent to start
oc rollout status deployment spire-agent-upstream -n zero-trust-workload-identity-manager --timeout=120s

# Check upstream agent attested to Hub:
oc logs -l app.kubernetes.io/name=spire-agent-upstream -n zero-trust-workload-identity-manager | grep "Node attestation was successful"
...
export KUBECONFIG=$HUB_KUBECONFIG
oc exec spire-server-0 -c spire-server -n zero-trust-workload-identity-manager -- /spire-server agent list | grep spoke01
...
AGENT_ID=$(oc exec spire-server-0 -c spire-server -n zero-trust-workload-identity-manager -- /spire-server agent list 2>&1 | grep "k8s_psat/spoke01" | head -1 | awk '{print $NF}')
echo "Upstream agent SPIFFE ID: $AGENT_ID"
...
oc wait --for=condition=Ready pod/spiffe-chain-verify -n spiffe-demo --timeout=120s
...
```
┌─── Hub Cluster (AWS STS) ──────────┐       ┌─── Fed Cluster (GCP) ─────────────┐
│                                    │       │                                    │
...
3. **IRSA with projected ServiceAccount tokens** — The aws-privateca-issuer pod authenticates to AWS via IRSA (IAM Roles for Service Accounts). An IAM role with a trust policy scoped to the pod's ServiceAccount + the cluster's OIDC provider allows the AWS SDK to call `sts:AssumeRoleWithWebIdentity` automatically. No static credentials, no Secrets, no rotation. The worker node role is not modified.
...
| CertificateRequest stuck in Pending (approved but not signed) | aws-privateca-issuer can't reach AWS PCA      | Verify IAM role trust policy, check projected token mount exists, check controller logs: `oc logs -n aws-privateca-issuer deploy/aws-pca-issuer-aws-privateca-issuer` |
...
2. **No credential rotation needed** — IRSA uses short-lived STS tokens (1-hour TTL, auto-refreshed by the AWS SDK). No static secrets to rotate.
...
4. **Cleanup** — To fully delete the PCA: `aws acm-pca delete-certificate-authority --certificate-authority-arn $PCA_ARN --permanent-deletion-time-in-days 7`. Also delete the IAM role: `aws iam detach-role-policy --role-name spire-pca-issuer-role --policy-arn $POLICY_ARN && aws iam delete-role --role-name spire-pca-issuer-role && aws iam delete-policy --policy-arn $POLICY_ARN`

---

## Quick Reference: Cluster Kubeconfigs

```bash
# Set all three at once (copy-paste block)
export HUB_KUBECONFIG=<path-to-hub-cluster-kubeconfig>
export SPOKE_KUBECONFIG=<path-to-spoke-cluster-kubeconfig>
export FED_KUBECONFIG=<path-to-fed-cluster-kubeconfig>
```
